...
<html lang="en" dir="ltr">
  ...
  <body>
	  ...
	  <br>
		...
		<br>
		<div id="result"></div> <!--reult will get displayed--> 
      </div>
	  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
	  <script> // when we press a key then annoymous function inside keyup will execute & txt will now have the value which is entered inside the search members
		$('#search_text').keyup(function() {
			var txt = $(this).val();
			if(txt!='') // if txt is not empty then we are calling the ajax function
			{ 
				$.ajax({ // inside this there is a json object
					url: "search.php",// executesearch.php
					method: "post", // in post array at position search_string we are entering the search txt
					data: {search_string:txt},// search_string can be anything
					dataType: "text", // // in our case datatpe is string.
					success: function(data) // if search.php executes then anonymous function should execute.
					{
						$('#result').html(data); // on successfull execution it will have html code init & all the value of echo
					}
				});
			}
			else
