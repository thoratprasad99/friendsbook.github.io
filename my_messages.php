  ...
  <body>
	  ...
	  <br>
		...
		<div id="timeline">
				...
				<?php
				$user_id=$_GET['u_id'];
				$query  = "SELECT * FROM messages WHERE sender_id='$user_id' OR receiver_id='$user_id' ORDER BY 1 DESC";// assuming user id as 160 , all the messages will be selected where user id with 160 is either sender or receiver
				$result = $conn->query($query); 
				$rows = $result->num_rows;
				
				$x = array();			
				for ($j = 0 ; $j < $rows ; ++$j) // checking for otherperson & checking for the latest msg id & otherperson id & storing them 
				{
				$result->data_seek($j);
				$row = $result->fetch_array(MYSQLI_NUM);
		
				if($user_id==$row[1]) // these 2 if statements help to find the actual sender & receiver
					$otherperson = $row[2];
				if($user_id==$row[2])
					$otherperson = $row[1];
		
				if(!in_array($otherperson, $x)) // it will check if otherperson is present inside the array or not. It will be true if otherpersonis not there in the array. Eg.161 will become otherperson
					$x[$row[0]] = $otherperson;
				}
				...
				if(array_key_exists($row1[0], $x))// till this we got the latest msg between the sender & receiver & also we get to know the msg_id exists or not		
				{
				$i=$row1[0];
				$u=$x[$i];
